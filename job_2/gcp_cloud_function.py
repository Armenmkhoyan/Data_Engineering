...
def validate_line(line):
    return all([validate_url(line["url"]), is_digit(line["id"]), is_digit(line["creation_timestamp"]),
...
def create_bucket(client, bucket_name, location="US"):
    ...
    if not is_bucket_exist(client, bucket_name):
        try:
            return client.create_bucket(bucket_name, location=location, requester_pays=False)
